...
writer = SummaryWriter("experiment_original")
...
def train(epochs):
    ...
    for batchid,(input_,label_) in enumerate(trainloader):
        # print(batchid)
        input_,label_=input_.to(device),label_.to(device)

        optimizer.zero_grad()
        outputs=EFnet(input_)
...
def test(epoch):
    ...
    total = 0
    with torch.no_grad():
        for batchid,(input_,label_) in enumerate(testloader):
            # print(batchid)
            input_,label_=input_.to(device),label_.to(device)
